...
public class WaveProgressView extends View {
    ...
    //默认波纹颜色
    private static final int WAVE_PAINT_COLOR = 0x083D5AFE;
    //默认外环颜色
    private static final int OUTER_RING_COLOR = 0xff3D5AFE;
    // y = Asin(wx+b)+h
...
//        centerPaint.setColor(centerTextY > proHeight + textRect.height() ? (submergedTextColor == 0xffffff0 ? centerTextColor : submergedTextColor) : centerTextColor);
        ...
        canvas.drawText(centerText, width / 2.0f - textRect.width() / 2.0f, centerTextY + 20
                , centerPaint);
    ...
    public void setProgress(int progress) {
        ...
        String content = String.format("%.0f", (float) progress / max * 100) + "%";
        switch (bindingText) {
            case TOP_TEXT:
                topText = content;
                break;
