...
import com.etzSharding.utils.AppFilePath;
...
import static com.etzSharding.utils.Util.getNewMac;
...
//        TokenUtil.getTokenDatas();
        ...
        Bugly.init(getApplicationContext(), "f57525a0ca", false);
// 日志输出
        try {
            initOkGo();
        } catch (IOException e) {
...
//        if (app == null) app = SyncReceiver.app;
    ...
    public synchronized BtcConfiguration getConfiguration() {
        if (config == null)
            config = new BtcConfiguration(PreferenceManager.getDefaultSharedPreferences(this), getResources());
        return config;
    }
    ...
    public String getWallteAddress() {
        if (Util.isNullOrEmpty(addr)) {
            addr = SharedPrefsUitls.getInstance().getCurrentWalletAddress();
        }
        return addr;
    }
