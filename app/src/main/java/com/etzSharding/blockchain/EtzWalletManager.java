package com.etzSharding.blockchain;
...
import com.etzSharding.bean.RPCBalanceBean;
import com.etzSharding.bean.ResponseResultBean;
...
public class EtzWalletManager implements BaseWalletManager {
            ...
            public void run() {
                ...
                HttpRequets.postRequest(ethRpcUrl, getClass(), payload.toString(), new JsonCallback<RPCBalanceBean>() {
                    ...
                    public void onSuccess(Response<RPCBalanceBean> response) {
                        ...
                        BalanceDataSource.getInstance().insertTokenBalance(new BalanceEntity(wid + "ETZ", wid, "ETZ", bigb));
    ...
    public BigDecimal getFiatExchangeRate(Context app, String iso) {
        BigDecimal fiatData = getFiatForEth("ETZ", new BigDecimal(1), SharedPrefsUitls.getInstance().getPreferredFiatIso());
    ...
    private BigDecimal getFiatForEth(String iso, BigDecimal ethAmount, String code) {
        ...
        //Btc rate for ether
        CurrencyEntity ethBtcRate = RatesDataSource.getInstance(MyApp.getmInstance()).getCurrencyByCode(MyApp.getmInstance(), "ETZ", "BTC");
        if (btcRate == null) {
